...
namespace B1Task
{
    public abstract class FileGenerator
    {
        ...
        public static async Task GenerateFilesAsync(string folderPath, int fileCount, int linesPerFile, IProgress<int> progress)
        {
            await Task.Run(() =>
            {
                Parallel.For(0, fileCount, i =>
                {
                    ...
                    using (var stream = new FileStream(fileName, FileMode.Create, FileAccess.Write, FileShare.None, 4096, FileOptions.SequentialScan))
        ...
        private static string GenerateLine()
        {
            ...
            resultArray.Add(GenerateRandomString(10, "абвгдеёжзийклмнопрстуфхцчшщъыьэюяАБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯ", random));
