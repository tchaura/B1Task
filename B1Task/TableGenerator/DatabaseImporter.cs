...
public class DatabaseImporter(string connectionString)
{
    ...
    {
        var block = new ActionBlock<List<string>>(async lines => { await InsertBatchAsync(lines, progress); },
        ...
        using (var reader = new StreamReader(filePath))
        {
            ...
            if (batch.Count > 0) await block.SendAsync(batch);
        }
    ...
    private async Task InsertBatchAsync(List<string> lines, IProgress<int> progress)
    {
        const string copyCommand =
            "COPY DataTable (date, latintext, cyrillictext, evennumber, floatingnumber) FROM STDIN (FORMAT BINARY)";
        ...
        foreach (var line in lines)
        {
            ...
            await writer.WriteAsync(DateTime.ParseExact(parts[0], "dd.MM.yyyy", CultureInfo.InvariantCulture),
                NpgsqlDbType.Date);
            await writer.WriteAsync(parts[1], NpgsqlDbType.Text);
            await writer.WriteAsync(parts[2], NpgsqlDbType.Text);
            await writer.WriteAsync(long.Parse(parts[3]), NpgsqlDbType.Integer);
